...
<section class="dark:bg-gray-900 p-3 sm:p-5">
      <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
          ...
          <div class="p-10 bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            ...
            <form action="{{ route('pengeluaran.update', $pengeluaran->kode_t) }}" method="POST">
              @csrf
              @method('PUT')
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                   <div>
                        <label for="kode_t" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Kode Transaksi</label>
                        <input name="kode_t" type="number" id="kode_t" value="{{ old('kode_t', $pengeluaran->kode_t) }}" class="@error('kode_t') border-red-500 @enderror bg-gray-50 border text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Masukan Kode Transaksi" required>
                        @error('kode_t')
                    ...
                    <div>
                        ...
                        <input name="transaksi" type="text" id="transaksi" value="{{ old('transaksi', $pengeluaran->transaksi) }}" class="@error('transaksi') border-red-500 @enderror bg-gray-50 border text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Masukan Transaksi" required>
                        @error('transaksi')
                          <p class="text-red-500 text-xs">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input name="total" type="number" id="total" value="{{ old('total', $pengeluaran->total) }}" class="@error('total') border-red-500 @enderror bg-gray-50 border text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Masukan Total" required>
                        @error('total')
                          <p class="text-red-500 text-xs">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input name="keterangan" type="text" id="keterangan" value="{{ old('keterangan', $pengeluaran->keterangan) }}" class="@error('keterangan') border-red-500 @enderror bg-gray-50 border text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Masukan Keterangan" required>
                        @error('keterangan')
                          <p class="text-red-500 text-xs">{{ $message }}</p>
                        @enderror
                    </div>
