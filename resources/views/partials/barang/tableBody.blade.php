		...
		<p class="w-full text-center mt-4">No Data Found!</span>
			...
			<thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
				<tr>
					...
					<th scope="col" class="px-4 py-3">HPP</th>
			...
			<tbody>
				...
				<tr class="border-b dark:border-gray-700">
					...
					<td class="px-4 py-3">{{ $item->hpp }}</td>
					<td>
						<form class="px-4 py-3 flex items-center justify-center gap-4"
							onsubmit="return confirm('Apakah Anda Yakin?');"
							action="{{ route('barang.destroy', $item->kode_barang) }}" method="POST">
							<a href="{{ route('barang.edit', $item->kode_barang) }}"><svg xmlns="http://www.w3.org/2000/svg"
