    ...
    <p class="w-full text-center mt-4">No Data Found!</span>
      ...
      <tbody>
        ...
        <tr class="border-b dark:border-gray-700">
          ...
          <td class="px-4 py-3">{{ $item->no_tlp }}</td>
          <td>
            <form class="px-4 py-3 flex items-center justify-center gap-4"
              onsubmit="return confirm('Apakah Anda Yakin?');" action="{{ route('distributor.destroy', $item->ID) }}"
              method="POST">
              <a href="{{ route('distributor.edit', $item->ID) }}"><svg xmlns="http://www.w3.org/2000/svg" fill="none"
