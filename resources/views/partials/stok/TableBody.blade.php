  ...
  <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
    ...
    <p class="w-full text-center mt-4">No Data Found!</span>
      ...
      <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
        <tr>
          <th scope="col" class="px-4 py-3">Kode Stok</th>
      ...
      <tbody>
        ...
        <tr class="border-b dark:border-gray-700">
          <td class="px-4 py-3">{{ $item->kode_stok }}</td>
          <td class="px-4 py-3">{{ $item->jumlah_stok }}</td>
          <td>
            <form class="px-4 py-3 flex items-center justify-center gap-4"
              onsubmit="return confirm('Apakah Anda Yakin?');" action="{{ route('stok.destroy', $item->kode_stok) }}"
              method="POST">
              <a href="{{ route('stok.edit', $item->kode_stok) }}"><svg xmlns="http://www.w3.org/2000/svg" fill="none"
